...
impl<Pk: MiniscriptKey, Ctx: ScriptContext, T> PushAstElem<Pk, Ctx> for script::Builder<T> {
    fn push_astelem(self, ast: &Miniscript<Pk, Ctx>) -> Self
    where
        Pk: ToPublicKey,
    {
...
impl<Pk: MiniscriptKey, Ctx: ScriptContext, T> Terminal<Pk, Ctx> {
    ...
    /// `parse` module.
    pub fn encode(&self, mut builder: script::Builder<T>) -> script::Builder<T>
    where
        Pk: ToPublicKey,
    {
        match *self {
            ...
            Terminal::Hash160(ref h) => builder
                ...
                .push_opcode(opcodes::all::OP_EQUAL),
            Terminal::True => builder.push_opcode(opcodes::all::OP_TRUE),
            Terminal::False => builder.push_opcode(opcodes::all::OP_FALSE),
