...
use bitcoin::script::{self, PushBytes, ScriptSigTag};
use bitcoin::{Address, Network, ScriptBuf, Weight, ScriptPubKeyBuf, ScriptSigBuf};
...
impl<Pk: MiniscriptKey + ToPublicKey> Pkh<Pk> {
        ...
        S: Satisfier<Pk>,
    {
        if let Some(sig) = satisfier.lookup_ecdsa_sig(&self.pk) {
            let script_sig = script::Builder::<ScriptSigTag>::new()
